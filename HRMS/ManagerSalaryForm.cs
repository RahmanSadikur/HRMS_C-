...
namespace HRMS
{
    public partial class ManagerSalaryForm : Form
    {
        HRMSdbContext1 context = new HRMSdbContext1();
        public ManagerSalaryForm()
        {
            InitializeComponent();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtbasic.Text == "")
            { MessageBox.Show("please insert Basic"); }
            if (txtempid.Text == "")
            { MessageBox.Show("please insert employee id"); }
            if (dtpsal.Value == DateTime.Now)
            { MessageBox.Show("please insert Date na"); }
        ...
        {
            var sal = context.salaries.Where(s=>s.employee.usertype.utypeId==3).ToList();
            int id1;
